  ...
  <v-sheet class="my-10">
    ...
    <v-container>
      <v-tabs v-model="tab" color="primary">
        <v-tab :value="product.id" v-for="product in products" :key="product.id" :class="cardTitleClass">
          <v-icon start>
            {{ product.icon }}
          </v-icon>
          {{ product.name }}
      ...
      <v-window v-model="tab">
        <v-window-item :value="product.id" v-for="product in products" :key="product.id">
          <v-card flat >
            ...
            <v-card-actions>
              <v-btn :href="product.href" :class="cardActionClass">{{ $t("learnMore") }}
    ...
    return [
      {
        ...
        description: "Hi-Motor Designer 是新一代电机设计智能助手。\
          ...
          探索高性能电机的奥秘。",
        href: "https://designer.hi-motor.site",
        imgUrl: productHMDZh,
      },
      {
        id: 2,
